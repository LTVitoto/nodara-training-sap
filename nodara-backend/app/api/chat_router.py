...
async def process_chat(payload: ChatInput):
    ...
    if current_index >= len(preguntas_db): 
        return {"response": "Exame concluído com sucesso."}
    ...
    inputs = {
        ...
        "total_questions": len(preguntas_db),
        "source_manual_language": "English", 
        "target_training_language": target_lang,
        "target_sap_process": payload.exam_id,
        "active_question_text": preguntas_db[current_index][1], 
        "active_rubric_json": json.loads(preguntas_db[current_index][2]),
    ...
    try:
        # En producción, esto capturará todos los eventos reales de SAP AI Core
        langfuse_handler = CallbackHandler()
        
        result = nodara_graph.invoke(
            inputs, 
            config={"callbacks": [langfuse_handler], "configurable": {"thread_id": session_key}}
        )
